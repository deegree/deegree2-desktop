...
public class ExifModule<T> extends DefaultModule<T> {
    ...
    public void addImages() {
        if ( appContainer.getViewPlatform().equalsIgnoreCase( "application" ) ) {
            ...
            if ( chooser.showOpenDialog( ( (DeegreeDesktop) appContainer ).getMainWndow() ) == JFileChooser.APPROVE_OPTION ) {
                ...
                if ( !errors.isEmpty() ) {
                    DialogFactory.openWarningDialog( appContainer.getViewPlatform(),
                                                     ( (DeegreeDesktop) appContainer ).getMainWndow(),
        ...
        try {
            ...
            if ( metadata instanceof JpegImageMetadata ) {
                ...
                DialogFactory.openErrorDialog( appContainer.getViewPlatform(),
                                               ( (DeegreeDesktop) appContainer ).getMainWndow(), get( "$MD10898" ),
                                               get( "$DI10017" ) );
        ...
        if ( appContainer.getViewPlatform().equalsIgnoreCase( "application" ) ) {
            ...
            try {
                ...
                if ( visitor.col.size() == 1 ) {
                    ...
                    DialogFactory.openErrorDialog( appContainer.getViewPlatform(),
                                                   ( (DeegreeDesktop) appContainer ).getMainWndow(), get( "$MD10897" ),
                                                   get( "$DI10017" ) );
                }
            } catch ( IOException e ) {
                DialogFactory.openErrorDialog( appContainer.getViewPlatform(),
                                               ( (DeegreeDesktop) appContainer ).getMainWndow(), get( "$MD10899", e ),
                                               get( "$DI10017" ) );
            } catch ( Exception e ) {
                LOG.logError( "Unknown error", e );
            }
