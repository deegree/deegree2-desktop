...
import org.deegree.desktop.commands.geoprocessing.LayerIntersectionCommand;
...
import org.deegree.desktop.views.swing.geoprocessing.IntersectionModel;
...
public class IntersectionModule<T> extends DefaultModule<T> {
    ...
    public void intersect() {
        IntersectionModel im = (IntersectionModel) view;
        if ( im != null ) {
            Command command1 = new LayerIntersectionCommand( im.getMainLayer(), im.getCompareLayer(),
                                                             im.getNewLayerName(), im.getIntersectionType() );
            ...
            ProcessMonitor pm = ProcessMonitorFactory.createDialogProcessMonitor( appContainer.getViewPlatform(),
                                                                                  Messages.get( "$MD10582" ),
                                                                                  Messages.get( "$MD10583" ), 0, 1,
                                                                                  command1 );
