...
import org.deegree.desktop.commands.model.AddWMSLayerCommand;
            ...
            if ( srcButton.getName().equals( ButtonPanel.CANCEL_BT ) ) {
                ...
                if ( requestSeparately.isSelected() ) {
                    nextStep = new AddWMSWizardSummary( this, this.mapModel, this.appContainer, wmsCaps,
                                                        ...
                                                        getFormat(), isTransparent(), true, null, allowSwapAxis );
                } else {
                    nextStep = new AddWMSWizardSummary(
                                                        this,
                                                        this.mapModel,
                                                        this.appContainer,
                                                        wmsCaps,
                                                        CollectionUtils.unzip( singleton( CollectionUtils.zip(
                                                                                                               selectedLayers ).getFirst() ) ),
                ...
                if ( requestSeparately.isSelected() ) {
                    for ( Pair<Layer, String> pair : CollectionUtils.zip( selectedLayers ) ) {
                        AddWMSLayerCommand addWMSCmd = new AddWMSLayerCommand(
                                                                               this.mapModel,
                                                                               this.wmsCaps,
                                                                               CollectionUtils.unzip( singleton( pair ) ),
                                                                               pair.first.getName(), getFormat(),
                                                                               isTransparent(), allowSwapAxis );
