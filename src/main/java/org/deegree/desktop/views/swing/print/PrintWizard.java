...
import static org.deegree.desktop.views.swing.util.GuiUtils.unknownError;
...
import org.deegree.desktop.modules.PrintModule;
import org.deegree.desktop.views.swing.print.SelectTemplatePanel.Template;
import org.deegree.desktop.views.swing.util.wizard.Wizard;
    ...
    public void setInitParameters( PrintModule<T> module ) {
        try {
            ...
            if ( templates.size() == 1 ) {
                ...
                if ( !PrintActions.selectTemplateAction( fromList, reportPanel, optionsPanel, dpi ).forward() ) {
                    openErrorDialog( "Application", this, get( "$MD10351" ), get( "$DI10017" ) );
                    return;
                }
            } else {
                panels.add( fromList );
                actions.add( PrintActions.selectTemplateAction( fromList, reportPanel, optionsPanel, dpi ) );
            }

            panels.add( optionsPanel );
            actions.add( PrintActions.optionsAction( optionsPanel ) );

            panels.add( reportPanel );
            actions.add( PrintActions.printAction( reportPanel, mm, optionsPanel, dpi, wmsSizeScale ) );
