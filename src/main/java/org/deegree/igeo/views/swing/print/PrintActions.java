...
import org.deegree.igeo.views.swing.util.wizard.MiscActions;
...
public class PrintActions {
            ...
            public boolean forward() {
                if ( options.file == null && options.fileChanged ) {
                    ...
                    if ( options.file.exists()
                         && !DialogFactory.openConfirmDialogYESNO( "Application", options,
                                                                   get( "$DI10022", options.file.toString() ),
                                                                   get( "$DI10019" ) ) ) {
                        options.file = null;
                        return false;
                    }
                ...
                if ( options.file == null ) {
                    DialogFactory.openInformationDialog( "Application", options, get( "$MD10357" ), get( "$DI10018" ) );
                    return false;
                }
            ...
            public boolean forward() {
                ...
                try {
                    ...
                    if ( options.selectScale.isSelected() ) {
                        Envelope e = MapUtils.scaleEnvelope( originalExtent,
                                                             MapUtils.calcScale( mm.getTargetDevice().getPixelWidth(),
                    ...
                    if ( options.selectScale.isSelected() ) {
                        String s = options.scale.getSelectedItem().toString();
                        parameters.put( "SCALE", "1 : " + s );
                    } else {
                    ...
                    GuiUtils.unknownError( LOG, e, reportPanel ); // the template was already loaded
                    // before...
                } catch ( JRException e ) {
                    GuiUtils.showErrorMessage( reportPanel, get( "$MD10358", e.getMessage() ), e );
                    e.printStackTrace();
                } catch ( RenderException e ) {
                    GuiUtils.showErrorMessage( reportPanel, get( "$MD10359", e.getMessage() ), e );
                    e.printStackTrace();
                } catch ( Exception e ) {
                    GuiUtils.unknownError( LOG, e, reportPanel );
                    e.printStackTrace();
                }
                return true;
            }
            ...
            private BufferedImage renderMap( final MapModel mm, int width, int height, final List<Theme> themes,
                ...
                try {
                    MapView mv = createMapView( "iGeoDesktop", extent, extent.getCoordinateSystem(),
                                                tmp.toArray( new Theme[tmp.size()] ), MapUtils.DEFAULT_PIXEL_SIZE );
    ...
    static void calculatePrinterFriendlySize( Pair<Integer, Integer> pixels, int dpi ) {
        pixels.first = (int) Math.round( pixels.first / 72d * dpi );
        pixels.second = (int) Math.round( pixels.second / 72d * dpi );
    }
    ...
    public static Action selectTemplateAction( final SelectTemplatePanel fromList, final JasperReportPanel jasper,
        ...
        return MiscActions.chainAction( MiscActions.monitorJListAction( fromList.list ), new EmptyAction() {
            @Override
            public boolean forward() {
                ...
                try {
                    ...
                    GuiUtils.showErrorMessage( fromList, get( "$MD10360", e.getMessage() ), e );
                } catch ( SAXException e ) {
                    GuiUtils.showErrorMessage( fromList, get( "$MD10361", e.getMessage() ), e );
                } catch ( XMLParsingException e ) {
                    GuiUtils.showErrorMessage( fromList, get( "$MD10362", e.getMessage() ), e );
                }
                return false;
            }
        } );
