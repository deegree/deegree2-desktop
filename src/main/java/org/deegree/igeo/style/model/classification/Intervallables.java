...
public class Intervallables {
    ...
    public static class DateIntervallable implements Intervallable<Date> {
        ...
        public int hashCode() {
            if ( fHashCode == 0 ) {
                ...
                result = HashCodeUtil.hash( result, dateFormatter );
                result = HashCodeUtil.hash( result, defaultPattern);
                result = HashCodeUtil.hash( result, wrappee );
            }
            return fHashCode;
        }
    ...
    public static class DoubleIntervallable implements Intervallable<Double> {
        ...
        public int hashCode() {
            if ( fHashCode == 0 ) {
                int result = HashCodeUtil.SEED;
                result = HashCodeUtil.hash( result, decimalFormatter );
                result = HashCodeUtil.hash( result, defaultPattern);
                result = HashCodeUtil.hash( result, wrappee );
            }
            return fHashCode;
