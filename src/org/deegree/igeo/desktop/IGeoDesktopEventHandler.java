...
public class IGeoDesktopEventHandler {
    ...
    static void actionPerformed( IGeoDesktop igeo, ActionEvent event ) {
        JFrame frame = igeo.getMainWndow();
        ...
        if ( eventName.startsWith( "$class:" ) ) {
            ...
            try {
                ...
                throw new RuntimeException( Messages.getMessage( frame.getLocale(), "$DG10047" ) );
            }
            ...
            try {
                ...
                LOG.logError( e.getMessage(), e );
                throw new RuntimeException( Messages.getMessage( frame.getLocale(), "$DG10048" ) );
            }
            ...
            try {
                ...
                LOG.logError( e.getMessage(), e );
                throw new RuntimeException( Messages.getMessage( frame.getLocale(), "$DG10069", eventName,
                                                                 e.getMessage() ) );
            }
            ...
            LookAndFeelDialog laf = new LookAndFeelDialog( frame );
            try {
                if ( laf.confirmed() ) {
     ...
     * 
     */
    static void openNewProject( IGeoDesktop igeo ) {
        igeo.cleanUp();
        ...
        NewProjectSelectionDialog inst = new NewProjectSelectionDialog( igeo, igeo.getMainWndow(), prjt );
        if ( inst.getSelection() != null ) {
            try {
                ...
                igeo.paint();
                HelpFrame.reset();
                igeo.getMainWndow().setTitle( "new project" );
     ...
     * @param igeo
     */
    static void loadProject( final IGeoDesktop igeo ) {
        igeo.cleanUp();
        final JFrame frame = igeo.getMainWndow();
     ...
     */
    static void saveProject( IGeoDesktop igeo )
     ...
     * @throws JAXBException
     */
    static void saveProject( IGeoDesktop igeo, File file )
                            throws IOException, JAXBException {
        if ( file == null ) {
     ...
     * @param igeo
     */
    static void addMapModel( IGeoDesktop igeo ) {
        try {
     ...
     * @param igeo
     */
    static void removeMapModel( IGeoDesktop igeo ) {
     ...
     * @throws Exception
     */
    static void openMapModel( IGeoDesktop igeo ) {
        try {
     ...
     * @param action
     */
    static void invokeStaticEventHandler( IGeoDesktop igeo, String action ) {
        try {
            ...
            LOG.logError( e.getMessage(), e );
            JFrame frame = igeo.getMainWndow();
            DialogFactory.openWarningDialog( igeo.getViewPlatform(), frame, Messages.getMessage( frame.getLocale(),
                                                                                                 "$DI10011", action ),
                                             Messages.getMessage( frame.getLocale(), "$DI10012" ) );
        }
    }
     ...
     * @param actionName
     */
    static void invokeModuleEventHandler( IGeoDesktop igeo, String actionName ) {
        try {
            ...
            LOG.logError( e.getMessage(), e );
            JFrame frame = igeo.getMainWndow();
            DialogFactory.openWarningDialog( igeo.getViewPlatform(), frame,
                                             Messages.getMessage( frame.getLocale(), "$DI10013", actionName ),
                                             Messages.getMessage( frame.getLocale(), "$DI10014" ) );
        }

    }

    static void createNewProjectByDataFile( IGeoDesktop igeo ) {
        final JFrame frame = igeo.getMainWndow();
        try {
            ...
            String[] s = StringTools.toArray( Messages.getMessage( frame.getLocale(), "$MD11767" ), "|", false );
            File file = GenericFileChooser.showOpenDialog( FILECHOOSERTYPE.project, igeo, frame, prefs, "geoDataFile",
                                                           IGeoFileFilter.createForExtensions( s ) );
            if ( file != null ) {
                Envelope bbox = getBBox( file, frame );
                if ( bbox == null ) {
                    DialogFactory.openInformationDialog( igeo.getViewPlatform(), frame,
                                                         Messages.getMessage( frame.getLocale(), "$MD11768" ),
                                                         Messages.getMessage( frame.getLocale(), "$MD11769" ) );
                    return;
                }
                ...
                igeo.paint();
                HelpFrame.reset();
                frame.setTitle( Messages.getMessage( frame.getLocale(), "$MD11770" ) );
                frame.setCursor( CursorRegistry.DEFAULT_CURSOR );
            }
        } catch ( Exception e ) {
            LOG.logError( e.getMessage(), e );
            DialogFactory.openErrorDialog( "application", frame, Messages.getMessage( frame.getLocale(), "$MD11771" ),
                                           Messages.getMessage( frame.getLocale(), "$MD11772" ), e );
        }
     ...
     * @throws IOException
     */
    private static Envelope getBBox( File file, JFrame frame )
                            throws Exception {
        Envelope env = null;
        ...
        if ( env.getCoordinateSystem() == null ) {
            CRSChooserDialog crsChosser = new CRSChooserDialog( frame );
