...
import org.deegree.igeo.commands.WASLogInCommand;
...
import org.deegree.igeo.mapmodel.ComponentPosition.BorderPosition;
import org.deegree.igeo.mapmodel.ComponentPosition.SplitterPosition;
 ...
 */
public class IGeoDesktop extends ApplicationContainer<Container> implements ActionListener {
    ...
    private Map<Identifier, Map<String, List<Object>>> actionMap = new LinkedHashMap<Identifier, Map<String, List<Object>>>();

    private Map<String, List<AbstractButton>> toolbarButtons;

    Map<String, AbstractButton> menuItems;

    Container rootTargetPane = null;
    ...
    void cleanUp() {
        ...
        try {
            ...
            if ( getCommandProcessor().availableUndos().size() > 0 ) {
                if ( DialogFactory.openConfirmDialogYESNO( "application", frame,
...
//                frame.requestFocus();                  
     ...
     */
    private Container setTargetLayout( LayoutType layout, Container targetPane, List<IModule<Container>> modList,
     ...
     */
    private int[] determineGridSize( List<IModule<Container>> modules ) {
        int maxCol = -1;
     ...
     * @param targetPane
     */
    void appendModules( List<IModule<Container>> modules, Container targetPane ) {
    ...
    void addToolBarEntries( IModule<Container> module ) {
     ...
     */
    private void appendToolBar( IModule<Container> module, List<ToolbarEntryType> toolBarEntries ) {
     ...
     * @param button
     */
    private void registerAction( Identifier identifier, String action, AbstractButton button ) {
     ...
     * @param aButton
     */
    private void addIcon( OnlineResourceType icon, AbstractButton aButton, int size ) {
        if ( icon != null ) {
    ...
    private void addIcon( String href, AbstractButton aButton, int size ) {
        Image img = null;
        try {
    ...
    /**
     * performs a login by executing {@link WASLogInCommand}
     * 
     * @param user
     * @param password
     */
