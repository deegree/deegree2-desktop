    ...
    public void selectedFeaturesToLayer() {
        ...
        try {
            ...
            if ( visitor.col.size() > 0 ) {
                FeatureCollection col = createFeatureCollection( "UUID_" + randomUUID().toString(), visitor.col.size() );
            ...
            for ( Layer l : mm.getLayersSelectedForAction( MapModel.SELECTION_ACTION ) ) {
                for ( int i = 0; i < col.size(); ++i ) {
                    ...
                    if ( pair.first != null && pair.first.equals( f.getFeatureType() ) ) { // this is implemented on a
                        ...
                        if ( ( asked && yes )
                             || ( !asked && openConfirmDialogYESNO( appContainer.getViewPlatform(), getViewForm(),
        ...
        for ( DataAccessAdapter adapter : layer.getDataAccess() ) {
            if ( adapter instanceof FeatureAdapter ) {
                ...
                for ( PropertyType pt : pts ) {
                    if ( pt.getType() == GEOMETRY ) {
                        props.add( createFeatureProperty( pt.getName(), geometry ) );
                    } else {
                        for ( int i = 0; i < pt.getMinOccurs(); ++i ) {
                            switch ( pt.getType() ) {
                            case VARCHAR:
                                props.add( createFeatureProperty( pt.getName(), "string" ) );
                            case INTEGER:
                                props.add( createFeatureProperty( pt.getName(), 0 ) );
                            case DATE:
                                props.add( createFeatureProperty( pt.getName(), new Date() ) );
                            }
                ...
                return new InsertFeatureCommand( adapter, createFeature( randomUUID().toString(), ft, props ) );
            }
        }
