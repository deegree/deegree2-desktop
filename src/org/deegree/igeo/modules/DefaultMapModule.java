...
import static org.deegree.framework.util.CollectionUtils.last;
...
public class DefaultMapModule<T> extends DefaultModule<T> implements CommandProcessedListener {
    ...
    public void activateHyperlink() {
        ...
        jco.addMouseListener( new MouseAdapter() {
            ...
            public void mouseClicked( MouseEvent e ) {
                if ( mapTool.getState().getClass().equals( HotlinkState.class ) ) {
                    ...
                    if ( urls.size() > 1 ) {
                        val = (String) showInputDialog( null, get( "$MD10896" ), get( "$DI10019" ), QUESTION_MESSAGE,
                                                        null, urls.toArray(), val );
                        if ( val == null ) {
                            return;
                        }
                    }

                    String ext = last( val.split( "[.]" ) );
        ...
        final Cursor handCursor = getPredefinedCursor( HAND_CURSOR );
        final Cursor defaultCursor = getPredefinedCursor( DEFAULT_CURSOR );
        ...
        for ( Layer l : layers ) {
            for ( DataAccessAdapter adapter : l.getDataAccess() ) {
                if ( adapter instanceof FeatureAdapter ) {
                    ...
                    List<QualifiedName> refProps = map( fa.getDatasource().getDatasourceType().getReferenceProperty(),
                                                        toqualifiedname );
