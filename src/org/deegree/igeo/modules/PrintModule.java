...
public class PrintModule<T> extends DefaultModule<T> {
    ...
    static {
        ...
        ActionDescription ad4 = new ActionDescription( "directPrint", "starts printing current map onto an available print divice", null,
    ...
    public Vector<Template> extractTemplates()
        ...
        BufferedImage noimage = read( getApplicationContainer().resolve( map.get( "noTemplateImage" ) ) );
        ...
        for ( String s : map.get( "templates" ).split( "," ) ) {
            ...
            t.title = tmp[0].trim();
            if ( tmp.length == 3 ) {
                t.image = read( getApplicationContainer().resolve( tmp[2].trim() ) );
            } else {
                t.image = noimage;
            }
        }
