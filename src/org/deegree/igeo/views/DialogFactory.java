...
public class DialogFactory {
     ...
     */
    public static String openNewReferenceDialog( ApplicationContainer<?> appCont, Object parent, String message,
                                                 String reference, boolean asURL ) {
        String s = null;
        if ( "Application".equalsIgnoreCase( appCont.getViewPlatform() ) ) {
            NewReferenceDialog nud = new NewReferenceDialog( appCont, (Container) parent, message, reference );
            if ( asURL ) {
                s = nud.getAsURL();
            } else {
                s = nud.getAsFile();
